...
project.character <- function(x,
  ...
  if (method_string %in% c("GCV","REML")) {
    ...
    ## The initial dotargs go into the methodArgs reused in reprojections
    ## Later dotargs erase these initial dotargs... so do not test dotargs$quantreg which are NULL if nothin is to be changed
    methodArgs$keep.inbag <- identical(methodArgs$quantreg, TRUE) # creates keep.inbag the 1st times
    #  ... and overwrites TRUE with TRUE in reprojections...
    #
    methodArgs_for_reproject <- methodArgs # save 'original' copy (including evaluated dotargs) before further modifs
    #
  ...
  if (method_string=="ranger") {
    project_call$methodArgs <- methodArgs_for_reproject # no data, in particular, otherwise future data would not be taken into account.
    for (st in intersect(intersect(names(methodArgs_for_reproject), names(dotargs)), 
                         names(project_call))) 
      project_call[[st]] <- NULL
  }
...
.predictWrap_ranger <- function(oneprojector,newdata, use_oob=Infusion.getOption("use_oob"), 
                                is_trainset=FALSE, 
                                num.threads=NULL, 
                                ext_projdata, # doc'ed for project.character
                                quantiles=NULL,
                                ...) {
  if (is_trainset && use_oob) {
    if (is.null(quantiles)) {
      pred <- oneprojector$predictions # oob
    } else {
      if (is.null(num.threads)) num.threads <- max(1L,Infusion.getOption("nb_cores")) 
      # if ( is.null(dim(projdata <- oneprojector$call$data))) projdata <- ext_projdata
      # if ( is.null(projdata)) stop("Neither 'oneprojector$call$data' nor 'ext_projdata' are available.") # see doc project.character if problem
      # pred <- .predictOOBquant(oneprojector, quantiles=quantiles, training=projdata, 
      #                     ncores=num.threads)$quantiles[,1] # oob
      pred <- predict(oneprojector, num.threads=num.threads, # *without* data to get oob
    ...
    if (is.null(quantiles)) {
      ...
      pred <- predict(oneprojector,data=newdata,
                      num.threads=num.threads, type="quantiles", quantiles=quantiles,
                      ...)$predictions 
    }
    dim(pred) <- NULL # useful in quantiles case (in particular dropping colname...)
    # (2) In the second step we identify newdata that were in train set, where pred then 
    # contains inbag predictions, and replace such predictions by oob ones 
    if (use_oob &&  ! is.null(dim(newdata))) { 
      # A costly check will be performed so (is_trainset && use_oob) should be used whenever TRUE
      ...
      if (length(newinold)) {
        ...
        if ( ! is.null(quantiles)) {
          # predoob <- .predictOOBquant(oneprojector, quantiles=quantiles, training=projdata, 
          #                        ncores=num.threads)$quantiles[,1]
          predoob <- predict(oneprojector, num.threads=num.threads,  # *without* data
                             type="quantiles", quantiles=quantiles, ...)$predictions
          pred[oob_positions] <- predoob[na.omit(posinold)] # using out-of-bag predictions.
        } else pred[oob_positions] <- oneprojector$predictions[na.omit(posinold)] # using out-of-bag predictions.
...
.predictWrap <- function(oneprojector,newdata, use_oob=Infusion.getOption("use_oob"), 
  ...
  } else if (inherits(oneprojector,"projector")) { # defined by Infusion
    # list with 
    #   one of the other possible projector objects;
    #   parname and respname to handle cases where the two may differ;
    #   parent: the name of the parent projector used to construct the new response.   
    ## the nested .predictWrap() call will call .predictWrap_ranger()
    .predictWrap(oneprojector$projector,newdata=newdata, use_oob=use_oob, 
                 is_trainset=is_trainset, methodArgs=methodArgs, ext_projdata=ext_projdata, 
                 quantiles=oneprojector$quantile, ...)
  } else if (inherits(oneprojector,"ranger")) { 
    .predictWrap_ranger(oneprojector,newdata=newdata, use_oob=use_oob, 
                        is_trainset=is_trainset, quantiles=quantiles,
                        ext_projdata=ext_projdata, # doc'ed for project.character
...
.project_reftable_raw <- function (x, projectors, use_oob=Infusion.getOption("use_oob"), is_trainset=FALSE, 
  ...
  if (is.vector(x)) {
    ly <- unlist(ly)
           ...
           ## this comment seems wrong (matrix in slik_j case)
    ly <- do.call(cbind, ly)
    # check_raw_stats(ly, colnames(ly),remove = FALSE, verbose=0.5)
...
neuralNet <- function(formula,data) {
  ...
  train <- .get_wrap("train",pack="caret")
  train(form=formula, data=data, method='nnet', linout=TRUE, trace = FALSE)
}
  ...
  if (inherits(object,"ranger")) {
    ...
    if (is.null(parm)) {
      if (missing(proj)) {
        ...
        if (is.null(parm)) parm <- proj # for case projector$npp_opt=="quantiles"
      }
    } else {
      projnames <- ls(projectors)
    ...
    # Single projector has been extracted; 
    ...
    if ( is.null(new_rawdata)) {
      ...
      logLs <- object$logLs
      if (inherits(projector,"projector") && projector$npp_opt=="quantiles") {
        parentproj <- projectors[[projector$parent]]
        parentparm <- attr(parentproj,"project_call")$x
        y_parm <- logLs[,parentparm]-logLs[,projector$parent]
      } else y_parm <- logLs[,parm] # parm value
      x_pred <- logLs[,proj] # predictions
      # ___F I X M E___? there is no $proj_trainset in the initial object
      ...
      if (inherits(projector,"projector") && projector$npp_opt=="quantiles") {
        parentproj <- projectors[[projector$parent]]
        parentparm <- attr(parentproj,"project_call")$x
        y_parent <- .predictWrap(parentproj, newdata=new_rawdata,
                              use_oob=use_oob, is_trainset=is_trainset, 
                              ext_projdata=object$projdata)
        y_parm <- new_rawdata[,parentparm] - y_parent
      } else y_parm <- new_rawdata[,parm]
      x_pred <- .predictWrap(projector, newdata=new_rawdata,
                        use_oob=use_oob, is_trainset=is_trainset, 
                        ext_projdata=object$projdata)
      oob_positions <- attr(x_pred,"oob_positions")
      attr(x_pred,"oob_positions") <- NULL
    }
  }
...
plot_importance <- function (object, parm, proj, n.var = 30L, xlim=NULL, xlab = "Variable Importance", ylab = "", main="",
  ...
  if (inherits(object,"ranger")) {
    ...
    projectors <- object$projectors
    if (missing(proj)) {
      proj_info <- .proj_from_parm(projectors, parm)
      if (proj_info$is_projector) {
        projector <- projectors[[proj_info$proj]]$projector 
      } else projector <- projectors[[proj_info$proj]]
    } else {
      projector <- projectors[[proj]] # proj arg needed here
      if (inherits(projector,"projector")) projector <- projector$projector 
    }
  }
  imp <- projector$variable.importance
  if (is.null(imp)) {
    warning('No variable importance measure in ranger object (importance = "none"?)')
    return(NULL)
  } else {
